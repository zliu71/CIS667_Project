...
class RubiksState(object):
    def __init__(self):
        self.cube = np.zeros((3,3,3,3,6), dtype = int)
...
def initial_state():
    ...
    state.cube[2,:,:,0,1] = 1 #O
    state.cube[:,0,:,1,2] = 1 #W
    state.cube[:,2,:,1,3] = 1 #Y
    state.cube[:,:,2,2,4] = 1 #G
...
def A_star_solve_score(state, step, dpth): 
    cubies, match = cubie_match(state)
    score = 0
    depth = dpth
    path = []
...
def heuristic(state, score, depth, match, path):
    ...
    #for child in [state.rotx(), state.rot2x(), state.rotxp(), state.roty(), state.rot2y(), state.rotyp(), state.rotz(), state.rot2z(), state.rotzp()]:
        ...
        h_temp = heuristic(child, (score + (27 - mtch)), (depth - 1), mtch, path_temp) 
        flag = flag + 1
        if(h_temp != []):
            h.extend(h_temp)
        #print ("label1")
...
def random_process_score(state, step, score = 0): 
    cubies, match = cubie_match(state)
    if(match == 27):
        return score
    if(step == 0):
        return score
    return random_process_score(random.choice([state.rotx(), state.rot2x(), state.roty(), state.rot2y(), state.rotz(), state.rot2z()]), step - 1, score + (27 - match))
